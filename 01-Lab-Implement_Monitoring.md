...
**Lab Manual** *for the 17 October 2023 event.*
...
1. From the Cloud Shell pane, run the following to create the first virtual network and deploy a virtual machine into it by using the template and parameter files you uploaded:
    ...
    *Please create a passoword with minimum 12 char with atleast one small letter, one capital, one number, one special char.*
...
1. On the **Basics** tab of the **Create Log Analytics workspace** blade, enter the following settings, click **Review + Create** and then click **Create**:
    ...
    | Resource group | the name of a new resource group **yourname-11-rg1** |
...
1. On the **Create an Automation Account** blade, specify the following settings, and click **Review + Create** upon validation click **Create**:
    ...
    | Subscription | the name of the Azure subscription you are using in this lab |
    | Resource group | **yourname-11-rg1** |
...
1. In the Azure portal, search for and select **Virtual machines**, and on the **Virtual machines** blade, click **yourname-11-vm0**.

1. On the **yourname-11-vm0** blade, in the **Monitoring** section, click **Metrics**.

1. On the **yourname-11-vm0 \| Metrics** blade, on the default chart, note that the only available **Metrics Namespace** is **Virtual Machine Host**.
...
1. On the **yourname-11-vm0** blade, in the **Monitoring** section, click **Diagnostic settings**.

1. On the **Overview** tab of the **yourname-11-vm0 \| Diagnostic settings** blade, select a **Diagnostic storage account**, and then click **Enable guest-level monitoring**.

    >**Note**: Wait for the diagnostic settings extension to be installed. This might take about 3 minutes.

1. Switch to the **Performance counters** tab of the **yourname-11-vm0 \| Diagnostic settings** blade and review the available counters.
...
1. Switch to the **Logs** tab of the **yourname-11-vm0 \| Diagnostic settings** blade and review the available event log collection options.
...
1. On the **yourname-11-vm0** blade, in the **Monitoring** section, click **Logs** and then click **Enable**.

1. On the **yourname-11-vm0 - Logs** blade, ensure **Azure Monitor agent (Recommended)** is selected, and then click **Configure**.  
...
1. On the **yourname-11-vm0 \| Logs** blade, in the **Monitoring** section, click **Metrics**.

1. On the **yourname-11-vm0 \| Metrics** blade, on the default chart, note that at this point, the **Metrics Namespace** drop-down list, in addition to the **Virtual Machine Host** entry includes also the **Guest (classic)** entry.
...
1. On the **Select a scope** blade, on the **Browse** tab, navigate to the **yourname-11-rg0** resource group, expand it, select the checkbox next to the **yourname-11-vm0** virtual machine entry within that resource group, and click **Apply**.

    >**Note**: This gives you the same view and options as those available from the **yourname-11-vm0 - Metrics** blade.

1. In the **Metric** drop-down list, select **Percentage CPU**, in the **Aggregation** drop-down list, select **Avg**, and review the resulting chart.

1. On the **Monitor \| Metrics** blade, on the **Avg Percentage CPU for yourname-11-vm0** pane, click **New alert rule**.
...
1. On the **Basics** tab of the **Create action group** blade, specify the following settings (leave others with their default values) and select **Next: Notifications >**:
    ...
    | Subscription | the name of the Azure subscription you are using in this lab |
    | Resource group | **yourname-11-rg1** |
    | Action group name | **yourname-11-ag1** |
    | Display name | **yourname-11-ag1** |
...
1. On the **yourname-11-vm0** blade, click **Connect**, in the drop-down menu, click **RDP**, on the **Connect with RDP** blade, click **Download RDP File** and follow the prompts to start the Remote Desktop session.
...
1. From the Command Prompt, run the following to trigger increased CPU utilization on the **yourname-11-vm0** Azure VM:
...
1. Use **CpuStress v2.0** - a Sysinternal tool to simulate CPU usage from https://learn.microsoft.com/en-us/sysinternals/downloads/cpustres


2. Leave the Remote Desktop session open and switch back to the browser window displaying the Azure portal on your lab computer.

3. In the Azure portal, navigate back to the **Monitor** blade and click **Alerts**.

4. Note the number of **Sev 3** alerts and then click the **Sev 3** row.

    >**Note**: You might need to wait for a few minutes and click **Refresh**.

5. On the **All Alerts** blade, review generated alerts.
...
1. If necessary, click **Select scope**, on the **Select a scope** blade, select the **Recent** tab, select **yourname-11-vm0**, and click **Apply**.
...
1. List all resource groups created throughout the labs of this module by running the following command:
   ...
   ```powershell
   Get-AzResourceGroup -Name 'yourname-11*'
   ```

1. Delete all resource groups you created throughout the labs of this module by running the following command:

   ```powershell
   Get-AzResourceGroup -Name 'yourname-11*' | Remove-AzResourceGroup -Force -AsJob
   ```
